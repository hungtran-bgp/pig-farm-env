...
global {
    ...
    action clear_dir {
    	bool delete_folder <- delete_file("../includes/output/vn_rena/autumn/"+ experiment_id);
    	file data <- new_folder("../includes/output/vn_rena/autumn/"+experiment_id);
    }
}

experiment RA_VN type:gui {
    	...
    	ask simulations {
    		loop pig over: RenaPig {
    			save [
    				...
    				pig.weight,
    				ET
    			] to: "../includes/output/vn_rena/autumn/" + experiment_id + "/" + string(pig.id) + ".csv" rewrite: false format: "csv";	
    		}
		}		
    }
