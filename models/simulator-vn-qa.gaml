...
global {
    ...
    reflex update_climate when: mod(cycle, 60*24)=0 {
    	...
    	RH <- get_rnd_RH();
    	vel <- rnd(0.25,0.35) with_precision 2;
    }
    float get_rnd_RH {
    ...
    action clear_dir {
    	bool delete_folder <- delete_file("../includes/output/vn_qui/autumn/"+ experiment_id);
    	file data <- new_folder("../includes/output/vn_qui/autumn/"+experiment_id);
    }
}

experiment QA_VN type:gui {
	parameter "Experiment ID" var: experiment_id <- "8";
	output {
    	...
    	ask simulations {
    		loop pig over: QuiPig {
    			save [
    				...
    				pig.weight,
    				ET
    			] to: "../includes/output/vn_qui/autumn/" + experiment_id + "/" + string(pig.id) + ".csv" rewrite: false format: "csv";	
    		}
		}		
    }
//    reflex capture when: mod(cycle, speed) = 0 {
//    	ask simulations {
//    		save (snapshot(self, "Simulator", {500.0, 500.0})) to: "../includes/output/rena/autumn/" + experiment_id + "-simulator-" + string(cycle) + ".png";
//    		save (snapshot(self, "CFI", {500.0, 500.0})) to: "../includes/output/rena/autumn/" + experiment_id + "-cfi-" + string(cycle) + ".png";
//    		save (snapshot(self, "Weight", {500.0, 500.0})) to: "../includes/output/rena/autumn/" + experiment_id + "-weight-" + string(cycle) + ".png";
//    		save (snapshot(self, "CFIPig0", {500.0, 500.0})) to: "../includes/output/rena/autumn/" + experiment_id + "-cfipig0-" + string(cycle) + ".png";
//    		save (snapshot(self, "DFIPig0", {500.0, 500.0})) to: "../includes/output/rena/autumn/" + experiment_id + "-dfipig0-" + string(cycle) + ".png";
//    	}
//    }
}
