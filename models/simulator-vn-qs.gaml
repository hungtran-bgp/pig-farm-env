...
global {
    ...
    action clear_dir {
    	bool delete_folder <- delete_file("../includes/output/vn_qui/summer/"+experiment_id);
    	file data <- new_folder("../includes/output/vn_qui/summer/"+experiment_id);
    }
}

experiment QS_VN type:gui {
    	...
    	ask simulations {
    		loop pig over: QuiPig {
    			save [
    				...
    				pig.weight,
    				ET
    			] to: "../includes/output/vn_qui/summer/" + experiment_id + "/" + string(pig.id) + ".csv" rewrite: false format: "csv";	
    		}
		}		
    }
